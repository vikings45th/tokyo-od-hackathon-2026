...
### 3-2. ブランチ運用 — **ブランチは作らない** ← ここが一番事故る

Claude Code は短時間に大きな差分を生みます。3人が同じファイルを触ると一瞬で衝突します。

#### 原則：全員 `main` に直接コミットする

残り時間は1.5日です。**ブランチを切る／PRを出す／レビューを待つ／マージする**の往復を
1人1日10回やると、それだけで数時間が溶けます。**衝突を防ぐ本体はブランチではなく
下のディレクトリ縦割り**なので、ブランチはもう仕事をしていません。やめます。

- **レビューは一切なし。** 自分の担当ディレクトリの中は自己責任で `main` に入れてよい
- **push は1〜2時間に1回以上。** クラウドセッションのVMは放置すると回収されます
- `main` に **Branch protection を掛けない**（レビュー必須にすると直 push が弾かれて成立しません）

**各自が最初に1回だけ実行するセットアップ**（コピペ）：
...
| 設定 | 何が起きるか |
|---|---|
| `pull.rebase` | マージコミットで履歴を汚さない |
| `rebase.autoStash` | **作業途中でも pull できる。**「commit してないから pull できない」が消えます |
| `git sync` | `git pull --rebase` → `git push` を1コマンドに |

**以降の作業ループはこれだけです：**

```bash
git add -A && git commit -m "..." && git sync
```
...
#### クラウドセッション（claude.ai/code）だけは例外

クラウドセッションは**毎回 `claude/xxx` ブランチが自動で切られる**ので、
main 直コミットができません。この場合のみ：

1. セッション内では、自動生成されたブランチにそのまま commit / push させる
2. **セッションが終わったら自分で `main` にマージする。他の2人を待たない**
   （GitHub で PR を作って、自分で Merge ボタンを押すだけ。Approve は不要です）
3. マージしたらそのブランチは削除する。次のセッションは新しいブランチから始まるので、
   古いブランチを引きずらない

> ⚠️ **クラウドセッションのブランチを2日間放置しないこと。**
> 溜めると 8/23 に巨大なブランチを3本同時マージすることになり、そこで詰みます。
> **その日のうちに `main` へ。**

#### 例外的にブランチを切るケース

大きな作り直しや、壊れるかもしれない実験のときだけ。名前は `wip/<名前>` の1本で足ります。
**寿命は数時間。** レビューなしで自分でマージして即消してください。
...
rebase の途中でコンフリクトを解くより圧倒的に速いのでこうしています。
...
**⛔ `git push --force` は絶対に使わないこと。** 3人が `main` を共有しているので、
1人の force push が他2人の作業を消します。`.claude/settings.json` でも拒否設定にしています。
