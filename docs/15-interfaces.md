...
| ファイル | 型 | 中身 |
|---|---|---|
| `data/app/munis.json` | `Muni[]` | 49自治体。公式推計・就学予定者・2023年度実績児童数・学童実績・注記 |
...
- **`Muni.baseChildren` は令和5年度（2023年度）の実績児童数（全学年）**。
  学童データ（令和5年5月1日）と基準年を揃えるためです（設計書 §4-3）
...
| | |
|---|---|
| 自治体 | **6件**：中央区・杉並区・世田谷区・品川区・江戸川区・三鷹市 |
...
| 自治体 | 実測値 | なぜ入れたか |
|---|---|---|
| **江戸川区** | 登録141人・クラブ4か所・登録率0.004 | **`excluded` の動作確認用。** ここを弾けているかで実装の正しさが分かる |
| **中央区** | 登録774人・15クラブ・**待機257人**・登録率0.088 | **`real-shortage` の動作確認用。** 除外してはいけない側 |
| **世田谷区** | 登録9,210人・**待機0** | **待機0でもリスクが0とは限らない**ことの確認用 |
| **杉並区** | 待機257人 | 課題仮説の実体験の舞台 |
| **品川区** | 登録2,979人・待機0 | スコア中位の確認用 |
| **三鷹市** | 登録2,289人・待機0 | 多摩市部の代表 |
