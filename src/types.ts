...
/** 学童データの外れ値の種別。3つを区別しないと本物の供給不足が消える（設計書 §4-4） */
export type NoteKind =
  /** 制度が別枠。スコア対象から外す。実測：江戸川区（登録141人・クラブ4か所） */
  | 'system-difference'
  /** 本物の供給不足。スコアに含める。実測：中央区（登録774人・待機257人） */
  | 'real-shortage'
...
export interface Muni {
  ...
  /** 2023年度の実績児童数（全学年）。r_latent の分母（設計書 §4-1 の N0） */
  baseChildren: number;
...
export interface Scenario {
  /**
   * 登録率の年あたり上昇幅。既定 0.015（＝+1.5pt/年）。
   *
   * 🔴 これがモデルを支配する（設計書 §0-2）。0 にすると49自治体中45が0点になる。
   *    都のオープンデータに実測値が無いため既定値は暫定。画面に「仮定」と明示すること。
   *    [-0.05, +0.05] にクランプする。
   */
  trend?: number;
