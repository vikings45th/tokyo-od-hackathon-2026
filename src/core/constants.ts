...
/** トレンド実測の起点。都計比の傾き（measureTrend）はこの2時点から引く */
export const GAKUDO_PREV_DATE = '2023-05-01';

/**
 * 顕在需要率（＝登録率）の年あたり上昇幅の既定値。**フォールバック時にもこの値を使う。**
 *
 * 実測：**48自治体 × 2時点（2023-05-01 → 2025-05-01）**、都計比で 0.2399 → 0.2562。
 * 2年で +0.0163 ＝ 年あたり **+0.0081**（`measureTrend(data/app/data.json).trend` = 0.008128）。
 * 対象は23区＋多摩26市の49自治体から、計上方法が変わった江戸川区（series-break）を除いた48。
 *
 * 🔴 この1つの値がモデルを支配する。画面でスライダーにすること（要件 NFR-5）。
 * 🔴 画面は `measureTrend(data).trend` を初期値に使う。**この定数と食い違わせないこと。**
 *    旧値 0.0084 は N0 訂正前の暫定値で、実データと 0.03pt ずれていた（docs/19 依頼8）。
 *    ズレの再発は `__tests__` が実データで検知する。
 */
export const DEFAULT_TREND = 0.0081;
